...
function resizeCanvas2() {
    const wrap = canvas2.parentElement;
    // Use the wrapper's dimensions as the source of truth
    const rect = wrap ? wrap.getBoundingClientRect() : null;
    const w = (rect && rect.width > 0) ? Math.round(rect.width) : (canvas2.clientWidth || 1000);
    const h = (rect && rect.height > 0) ? Math.round(rect.height) : (canvas2.clientHeight || 800);
...
function updateBgCache2() {
    ...
    if (window.isColorblindMode) {
        ctx.fillStyle = window.isDarkMode ? "rgba(255, 255, 0, 0.15)" : "rgba(139, 69, 19, 0.1)";
        ctx.strokeStyle = window.isDarkMode ? "rgba(255, 255, 0, 0.4)" : "rgba(139, 69, 19, 0.3)";
    } else {
...
function drawDynamicUI() {
    ctx2.save();
    const activeColor = window.isColorblindMode ? "#8b4513" : "#ff0055";
...
function draw2() {
    ...
    if (window.isDarkMode) {
        let activeHex = window.isColorblindMode ? "#8b4513" : "#ff0055";
        let activeRgb = window.isColorblindMode ? "139, 69, 19" : "255, 0, 85";

        coreColor = isInteracting ? activeHex : "#00f0ff";
        shadowColor = isInteracting ? `rgba(${activeRgb}, 0.8)` : "rgba(0, 240, 255, 0.8)";
    } else {
        let activeHex = window.isColorblindMode ? "#8b4513" : "#8b0000";

        coreColor = isInteracting ? activeHex : "#2c2c2c";
